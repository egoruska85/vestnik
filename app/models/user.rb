class User < ApplicationRecord
  ...
  def self.find_for_database_authentication(warden_conditions)
    ...
    elsif conditions.has_key?(:username) || conditions.has_key?(:email) || conditions.has_key?(:email)
      where(conditions.to_h).first
    end
  end

  def validate_username
    if User.where(email: username).exists? or User.where(email: phone).exists?
